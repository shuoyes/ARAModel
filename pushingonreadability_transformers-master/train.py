...
def _optimizer(args, model):
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)], 'weight_decay': 0.01},#包含所有需要应用权重衰减的参数。
        {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}#包含所有不需要应用权重衰减的参数。
    ]
    optimizer = AdamW(
        optimizer_grouped_parameters,
    ...
    for batch_idx, batch_item in enumerate(dataloader):
        ...
        progress.set_description(
            'Evaluaton: {:.3f}'.format(
                ...
                loss.detach().item()#loss.detach() 使该张量不再需要梯度，.item() 将张量转换为一个Python数字
            )
        )
        progress.update()
...
if args.model.lower() == 'bert':
    ...
    tokenizer_class = BertTokenizer
    model_class = BertForSequenceClassification
    pretrained_model_name = 'hfl/chinese-pert-large'
...
for k_cnt in range(num_kfold):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch_item in enumerate(train_loader):
            ...
            if (batch_idx + 1) % (len(train_loader) // args.n_eval_per_epoch) == 0:#(len(train_loader) // args.n_eval_per_epoch)表示每个评估所涵盖的批次数量，当(batch_idx + 1)和它相等时，即为做一次评估
                if args.do_evaluate:
                    dfs=pd.DataFrame()
                    train_acc, train_f1, train_p, train_r, train_qwk,_ = evaluate_on(model, train_loader)
                    valid_acc, valid_f1, valid_p, valid_r, valid_qwk,_ = evaluate_on(model, valid_loader)
